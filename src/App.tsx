...
export default function App() {
  ...
  return (
    <div className="bg-[#F8FAFC] text-slate-900 h-screen w-full flex flex-col overflow-hidden font-sans">
      
      {/* Top Navigation / Header */}
      <header className="h-16 border-b border-slate-200 flex items-center justify-between px-3 sm:px-6 bg-white shrink-0 z-20">
        <div className="flex items-center space-x-2 sm:space-x-3 min-w-0">
          <button 
            onClick={() => setSidebarOpen(!sidebarOpen)}
            className="p-1.5 text-slate-500 hover:text-slate-800 hover:bg-slate-50 rounded-sm shrink-0"
            title="Toggle Sidebar Menu"
          >
            <Menu className="w-5 h-5" />
          </button>
          
          <div className="w-8 h-8 bg-slate-900 flex items-center justify-center rounded-sm font-black text-[#FF9900] text-xs border border-slate-800 shrink-0">
            AWS
          </div>
          <div className="flex flex-col min-w-0">
            <span className="font-extrabold tracking-tight text-[11px] sm:text-xs md:text-sm text-slate-900 flex items-center gap-1 leading-none truncate">
              CLF-C02 STUDY COMPANION <GraduationCap className="w-3.5 h-3.5 text-amber-500 shrink-0 hidden xs:inline" />
            </span>
            <span className="text-[9px] text-slate-400 font-bold tracking-wide truncate hidden md:block">
        ...
        <div className="flex items-center space-x-1.5 sm:space-x-3 shrink-0 min-w-0">
          <div className="flex flex-col items-end shrink-0">
            ...
            <span className="text-xs sm:text-sm font-black text-slate-800 flex items-center gap-1 mt-0.5">
              {readinessScore}% 
              <span className={`w-2 h-2 rounded-full ${readinessScore >= 80 ? 'bg-emerald-500' : readinessScore >= 40 ? 'bg-amber-500' : 'bg-slate-300'}`}></span>
            </span>
          </div>
          
          <div className="w-px h-8 bg-slate-200 hidden xs:block"></div>

          <div className="px-2 py-0.5 bg-amber-50 text-amber-800 text-[10px] font-bold rounded-full border border-amber-100 items-center gap-1 hidden md:flex shrink-0">
          ...
          {authLoading ? (
            <div className="w-8 h-8 rounded-full bg-slate-100 animate-pulse border border-slate-200 shrink-0" />
          ) : user ? (
            <div className="flex items-center gap-1.5 shrink-0">
              {/* Cloud Sync Status Indicator */}
              <span 
                className="p-1 hover:bg-slate-50 rounded-sm text-slate-400 hover:text-slate-600 relative cursor-pointer"
              ...
              {/* User profile bubble */}
              <div 
                className="flex items-center gap-1.5 bg-slate-100 pl-1.5 pr-2.5 py-1 rounded-full border border-slate-200"
              ...
              >
                {user.photoURL ? (
                  ...
                  <div className="w-5 h-5 rounded-full bg-slate-800 text-[#FF9900] flex items-center justify-center font-bold text-[9px] border border-slate-700 shrink-0 uppercase">
                    {(user.displayName || user.email || 'A').charAt(0)}
                  </div>
                )}
                <span className="text-[10px] font-bold text-slate-700 max-w-[60px] truncate hidden xs:inline-block">
      ...
      <div className="flex-1 flex min-h-0 relative">
        ...
        <aside 
          className={`border-r border-slate-200 bg-white transition-all duration-300 ease-in-out shrink-0 flex flex-col justify-between 
        ...
        >
          ...
          <div className="flex md:hidden items-center justify-between p-4 border-b border-slate-100 bg-slate-50 shrink-0">
            <div className="flex items-center space-x-2">
              <div className="w-6 h-6 bg-slate-900 flex items-center justify-center rounded-sm font-black text-[#FF9900] text-[10px] border border-slate-800">
                AWS
              </div>
            ...
            <button
              onClick={() => setSidebarOpen(false)}
              className="p-1.5 text-slate-500 hover:text-slate-800 hover:bg-slate-100 rounded-sm"
              title="Close Menu"
            >
              <X className="w-5 h-5" />
            </button>
          ...
          <div className="p-4 space-y-6 flex-1 overflow-y-auto">
            <div>
              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-3 px-2">
                Core Workspace
              </span>
              <nav className="space-y-1">
                <button
                  onClick={() => handleTabChange("dashboard")}
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-sm text-xs font-bold tracking-tight transition-all text-left ${
                    activeTab === "dashboard"
            ...
            {sidebarOpen && (
              <div className="bg-slate-50/70 border border-slate-200/60 p-4 rounded-sm space-y-3">
                ...
                <div className="space-y-2">
                  <div>
                    <div className="flex justify-between text-[10px] text-slate-500 mb-1">
                      ...
                      <span className="font-bold">{knownCards} / {totalCards}</span>
                    </div>
                  ...
                  <div>
                    <div className="flex justify-between text-[10px] text-slate-500 mb-1">
                      ...
                      <span className="font-bold">{correctQuizzes} / {trickQuestions.length}</span>
                    </div>
                    <div className="w-full bg-slate-200 h-1.5 rounded-full overflow-hidden">
                      <div 
          ...
          <div className="p-4 border-t border-slate-100 text-[10px] text-slate-400 space-y-1 bg-slate-50/50 shrink-0">
            <p className="font-bold text-slate-500">CLF-C02 Premium Suite</p>
      ...
      {/* Footer Meta bar */}
      <footer className="h-10 bg-slate-100 border-t border-slate-200 px-6 flex items-center justify-between text-[10px] text-slate-400 shrink-0 font-mono select-none z-20">
        <div className="flex space-x-6 items-center">
          <span className="flex items-center gap-1.5">
            <Terminal className="w-3.5 h-3.5 text-slate-400" />
            SESSION: <span className="text-slate-600 font-semibold">AWS_MASTER_2026</span>
          </span>
          <div className="w-px h-3 bg-slate-300 hidden sm:block"></div>
          <span className="hidden sm:flex items-center gap-1.5">
            READINESS SCORE: 
            <span className="text-slate-700 font-bold">{readinessScore}%</span>
            <span className="w-16 bg-slate-200 h-1 rounded-full overflow-hidden inline-block border border-slate-300/40">
              <span 
        ...
        <div className="flex items-center gap-2">
          <span className="text-[#FF9900] font-bold uppercase tracking-widest text-[9px] flex items-center gap-1 bg-slate-200/50 px-2 py-0.5 rounded-sm border border-slate-300/40">
